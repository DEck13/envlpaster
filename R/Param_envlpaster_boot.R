Param_envlpaster_boot <- function(model, nboot, index, u, data, vectors, quiet = FALSE, m = 100, 
                                  using_method = c("1D_algorithm", "reducing_subspaces"), numCores) {
  ...
  if (using_method == "1D_algorithm") {
    ...
    r1 <- foreach(iboot= 1:nboot, .combine = "cbind") %dopar% {
    ...
    r2 <- foreach(iboot= 1:nboot, .combine = "cbind") %dopar% {
      #for(iboot in 1:nboot){
      library(aster2)
      library(envlpaster)
  ...
  if (using_method == "reducing_subspaces") {
    ...
    # inital quantities to save computing time
    G.star <- matrix(0, nrow = k, ncol = length(vectors)) 
    P.star <- matrix(0, nrow = k, ncol = k)
    xstar <- xstar2 <- matrix(0, nrow = n, ncol = nnode)
    ...
    # the MLE bootstrap
    r1 <- foreach(iboot= 1:nboot, .combine = "cbind",.inorder = TRUE) %dopar% {
      #for(iboot in 1:nboot){
      library(aster2)
      library(envlpaster)
      xstar2 <- raster(theta.hat2, pred, fam, root)
    ...
    # the envelope bootstrap
    r2 <- foreach(iboot= 1:nboot, .combine = "cbind", .inorder = TRUE) %dopar% {
      #for(iboot in 1:nboot){
      library(aster2)
      library(envlpaster)
      xstar <- raster(theta.hat, pred, fam, root)
    ...
    table <- cbind(tau.env, sqrt(diag(S)), tau[target.ind], 
                   sqrt(diag(S2)), ratio)
    colnames(table) <- c("env","se(env)","MLE","se(MLE)","ratio")  
    out <- list(u = length(vectors), table = table, S = S, S2 = S2, 
